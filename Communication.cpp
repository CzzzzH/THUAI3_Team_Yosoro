...
public:
	...
	bag() {
		gridient.resize(4, deque<DishType>{});//四个炉子
	}
...
vector<int> cook_time, generate_time;
vector<bool> is_fertilized, is_pot;
deque< deque<DishType> > gridient;
	...
	deque<DishType> gridient, int generation_id, int generate_time) 
{
	...
	message += (char)(cook_time % 256);
	message += (is_fertilized == true) ? 1 : 0;
	message += (is_pot == true) ? 1 : 0;
...
void UpdateInfo(std::string message, vector<int>& cook_time, 
	vector<int>& generate_time, vector<bool>& is_fertilized, vector<bool>& is_pot, bag& now_bag)
{
...
int main() {//测试函数功能
	...
	*/
	...
	cook_time = { 123,123,321,456 };
	generate_time = { 1,2,3,4 };
	is_fertilized = { true, true, false , false };
	is_pot = { true , true, false , false };
	deque<DishType> test2 = { (DishType)1,(DishType)2,(DishType)3,(DishType)25 };
	//string message = GenerateMessage(1, 1000, true, false, test2, 2, 1234);
	//UpdateInfo(message, cook_time, generate_time, is_fertilized, is_pot, test);
}
